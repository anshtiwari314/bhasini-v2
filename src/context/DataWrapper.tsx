...
export default function DataWrapper({children}:{children:React.ReactNode}) {
    ...
    const [socket3,setSocket3] = useState<any>(null)
    ...
    const [sourceLan,setSourceLan] = useState('hi')
    ...
    function sendAudioViaWS(blob:any,url:string,data:any){
        //console.log(blob)
        let reader = new FileReader()
        reader.onloadend = ()=>{
         ...
         socket3.emit('messagefromclient',JSON.stringify({
          audiomessage:base64data.split(',')[1],
          uid:data.id,
          sessionid:data.id,
          isadmin:data.isAdmin,
          mob:data.mob,
          init:data.init,
          source_language: data.sourceLan,
          target_language: data.targetLan,
          timeStamp:`${date.toLocaleDateString()} ${date.toLocaleTimeString()}:${date.getMilliseconds()}`,
...
// "sessionid": <str>, "audiofiletimestamp": <str>
    ...
    useEffect(()=>{
      ...
      let tempSocket3 = io('http://ec2-13-233-236-177.ap-south-1.compute.amazonaws.com/')
    ...
    useEffect(()=>{
        ...
        function send_message_listener(msg:string){
          console.log(msg)
          let temp = JSON.parse(msg)
          playAudio(temp?.audiomessage)
        }
        function onConnect(){
            console.log('socket3 connected')
          }
        socket3.on('messagefromserver',send_message_listener)
        socket3.on('connect',onConnect)
        return ()=>{
          socket3.off('connect',onConnect)
          socket3.off('messagefromserver',send_message_listener)
        }
    },[socket3])
...
//@ts-ignore
    ...
    useEffect(()=>{
        ...
        if(myStream===null || isHost===null ||users.length!=1 || socket===null || socket3===null)
        return ;
      ...
      function stop1(audio:any){
        ...
        downsampleToWav(wavBlob,(buffer:ArrayBuffer)=>{
            ...
            if(isHost ===true)
            sendAudioViaWS(blob,adminUrl,{...usersArrRef.current[0],init:false,sourceLan:'en',targetLan:'hi'})
            else
            sendAudioViaWS(blob,adminUrl,{...usersArrRef.current[0],init:false,sourceLan:'hi',targetLan:'en'})
    ...
    },[isHost,myStream,users,socket,adminUrl,socket3,sourceLan,targetLan])
    ...
    let values = {
        ...
        sourceLan,setSourceLan,
        targetLan,setTargetLan
    }   
